...
export default async (req, res, next) => {
  try {
    ...
    const totalDocuments = await Products.find({ type: { $in: ["MICROPHONE", "pc", "Chair","Mouse"] } }).countDocuments();
    pagination.page = parseInt(pagination.page);
    pagination.limit = parseInt(pagination.limit);

    const read_products = await Products.find({ type: { $in: ["MICROPHONE", "pc", "Chair","Mouse"] } })
