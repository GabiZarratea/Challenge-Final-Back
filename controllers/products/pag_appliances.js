...
export default async (req, res, next) => {
  try {
    ...
    const totalDocuments = await Products.find({ category: "Electrodomestics"}).countDocuments();
    pagination.page = parseInt(pagination.page);
    pagination.limit = parseInt(pagination.limit);

    const read_products = await Products.find({ type: { $in: ["Fridge", "TV", "Laundry", "Blender","Kitchen","Air"] } })
    ...
    return res.status(200).json({
      ...
      next: nextPage,
      currentPage: currentPage // Agregar el número de página actual a la respuesta
    });
  } catch (error) {
    console.log(error);
  }
